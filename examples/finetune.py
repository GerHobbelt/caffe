...
def style_net(train=True, learn_all=False, subset=None):
    ...
    source = os.path.join(caffe_root, 'data/flickr_style/%s.txt' % subset)
    transform_param = dict(mirror=train, crop_size=227,
        mean_file=os.path.join(caffe_root, 'data/ilsvrc12/imagenet_mean.binaryproto'))
    style_data, style_label = L.ImageData(
        transform_param=transform_param, source=source,
        # batch_size=50, new_height=227, new_width=227, ntop=2)
...
def main():
    ...
    style_label_file = os.path.join(caffe_root, 'examples/finetune_flickr_style/style_names.txt')
    with open(style_label_file) as f:
        style_labels = [line.strip() for line in f.readlines()]
    ...
    def eval_style_net(weights, test_iters=10):
        test_net = caffe.Net(style_net(train=False), caffe.TEST, weights=weights)
        accuracy = 0
