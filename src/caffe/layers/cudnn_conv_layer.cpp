...
void CuDNNConvolutionLayer<Dtype>::Reshape(
  ...
  for (int i = 0; i < bottom.size(); i++) {
    cudnn::setTensor4dDesc<Dtype>(&bottom_descs_[i], this->num_,
        this->channels_ / this->group_, height, width,
        this->channels_ * height * width, height * width, width, 1);
    cudnn::setTensor4dDesc<Dtype>(&top_descs_[i], this->num_,
        this->num_output_ / this->group_, height_out, width_out,
        this->num_output_ * this->out_spatial_dim_, this->out_spatial_dim_,
        width_out, 1);
    ...
    CUDNN_CHECK(cudnnGetConvolutionForwardAlgorithmMaxCount(
        handle_[0], &requested_algo_count));
    ...
    CUDNN_CHECK(cudnnGetConvolutionBackwardFilterAlgorithm_v7(handle_[0],
        ...
        requested_algo_count, &returned_algo_count, conv_bwd_filter_results));
    ...
    CUDNN_CHECK(cudnnGetConvolutionBackwardDataAlgorithm_v7(handle_[0],
        ...
        requested_algo_count, &returned_algo_count, conv_bwd_data_results));
...
#endif
    // get conv forward workspace size
    CUDNN_CHECK(cudnnGetConvolutionForwardWorkspaceSize(handle_[0],
        bottom_descs_[i], filter_desc_, conv_descs_[i], top_descs_[i],
        fwd_algo_[i], &(workspace_fwd_sizes_[i])));

    // get conv backward filter workspace size
    CUDNN_CHECK(cudnnGetConvolutionBackwardFilterWorkspaceSize(handle_[0],
        bottom_descs_[i], top_descs_[i], conv_descs_[i], filter_desc_,
        bwd_filter_algo_[i], &workspace_bwd_filter_sizes_[i]));

    // get conv backward data workspace size
